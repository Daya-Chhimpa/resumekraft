    ...
    switch (sectionKey) {
      ...
      case 'languages':
        return data.languages && data.languages.length > 0 ? (
          ...
          >
            <div className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 bg-white/80 px-2 py-1 rounded text-xs font-bold text-pink-500 pointer-events-none transition-opacity">Edit</div>
            <h3 className="font-bold mb-4 uppercase text-sm tracking-wider" style={{ fontFamily: fonts.heading, color: colors.text }}>Languages</h3>
            <div className="space-y-2">
              {data.languages.map(lang => (
                <div key={lang.id} className="flex justify-between text-xs" style={{ color: `${colors.text}90` }}>
                  <span>{lang.name}</span>
                  <span style={{ color: `${colors.text}60` }}>{lang.proficiency}</span>
                </div>
              ))}
            </div>
          </section>
